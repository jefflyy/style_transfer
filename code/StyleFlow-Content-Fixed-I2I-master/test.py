...
def main():
    ...
    for batch_id, (imgA, imgB, name) in enumerate(test_loader):
        ...
        c_rev = model(stylized, domain_class=encoder.cat_tensor(imgA.cuda()).cuda())

        output_name = os.path.join(args['output'], args['task_name'],'img_gen', name[0])
        save_image(torch.cat([imgA.cpu(), imgB.cpu(), stylized.cpu(), c_rev.cpu()], dim=1), output_name, nrow=1)
