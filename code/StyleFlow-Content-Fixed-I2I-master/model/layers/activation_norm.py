...
class SAN(nn.Module):
    def __init__(self,input_dim,output_dim):
        # print(f"SAN init {input_dim} {output_dim}")
    ...
    def forward(self, x, code):
        # print(f"SAN content:{x.shape} style:{code.shape}")
        ...
        print(f"before merge {x_mean.shape} {code_c_mean.shape}")
        ...
        # print(f"SAN result {mean.shape} {std.shape}")
        return mean,std
...
class RBMAD(nn.Module):
    def __init__(self,input_dim,output_dim):
        super(RBMAD, self).__init__()
        self.rb = Bottleneck(input_dim, input_dim, 3, 1)
        self.encoder = ToStyleHead(input_dim=input_dim, out_dim=768)
        self.code_encode_mean = nn.Linear(1920, 1024)
        self.act_code_encode_mean = nn.PReLU()
    ...
    def forward(self, x, code):
        x_rb = self.rb(x)
        x_enc = self.encoder(x_rb).flatten(1)

        code_c_mean = self.act_code_encode_mean(self.code_encode_mean(code))
        code_c_mean = self.act_code_encode_mean_nl(self.code_encode_mean_nl(code_c_mean))
        code_c_mean = torch.tanh(self.code_encode_mean_nl2(code_c_mean))

        x_mean = self.act_fc_mean_enc(self.fc_mean_enc(x_enc))
