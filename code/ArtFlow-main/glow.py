...
class AffineCoupling(nn.Module):
    def __init__(self, in_channel, filter_size=512, affine=True, swap=False):
        super().__init__()
    ...
    def reverse(self, output):
        ...
        return torch.cat([in_b, out_a] if self.swap else [out_a, in_b], 1)


class Flow(nn.Module):
    def __init__(self, in_channel, use_coupling=True, affine=True, conv_lu=True, swap=False):
        super().__init__()
        ...
        if self.use_coupling:
            self.coupling = AffineCoupling(in_channel, affine=affine, swap=swap)
...
class Block(nn.Module):
    def __init__(self, in_channel, n_flow, affine=True, conv_lu=True):
        ...
        for i in range(n_flow):
            self.flows.append(Flow(squeeze_dim, affine=affine, conv_lu=conv_lu, swap=(i % 2 == 1)))
...
class Glow(nn.Module):
    def __init__(self, in_channel, n_flow, n_block, stm, affine=True, conv_lu=True):
