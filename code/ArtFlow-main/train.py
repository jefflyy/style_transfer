...
stm_type = None
if args.operator == 'adain':
    stm_type = AdaIN
elif args.operator == 'rbmad':
    stm_type = RBMAD
else:
...
logger_path = os.path.join(args.save_dir, 'runs')
...
# glow
glow_single = Glow(3, args.n_flow, args.n_block, stm_type, affine=args.affine, conv_lu=not args.no_lu)
...
for i in range(args.start_iter, args.max_iter):
    current_lr = adjust_learning_rate(optimizer, iteration_count=i)
    ...
    logger.add_scalar('current_lr', current_lr, i + 1)
    logger.add_scalar('loss_c', loss_c.item(), i + 1)
    logger.add_scalar('loss_s', loss_s.item(), i + 1)
    logger.add_scalar('Loss', loss_style.item(), i + 1)

    # save image
    if i % args.print_interval == 0:
        output_name = os.path.join(args.save_dir, "%06d.jpg" % i)
        output_images = torch.cat((content_images.cpu(), style_images.cpu(), stylized.cpu()), 0)
